...
{ 
	...
	instanceData coop : SQUAD_PER_INSTANCE;	
...
struct VertexToPixel
{
	...
	float4 sColor		: COLOR;        // RGBA color
	float3 normal		: NORMAL;
	float3 viewDir		: DIR;
...
VertexToPixel main( VertexShaderInput input )
{
	...
	matrix worldViewProj = mul(mul(input.coop.worldMatr, view), projection);
	...
	output.normal = mul(input.normal, (float3x3)input.coop.worldMatr);

	worldPos = mul(float4(input.position,1.0f), input.coop.worldMatr);
	output.viewDir = camPos.xyz - worldPos.xyz;
	output.viewDir = normalize(output.viewDir);
	output.uv = input.uv;
	output.sColor = input.coop.surColor;
